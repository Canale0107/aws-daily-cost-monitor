# 手動セットアップガイド（管理者権限不足の場合）

## 🚨 問題

`canale.dev` ユーザーには以下の権限がありません：
- `iam:CreatePolicy`
- `iam:PutUserPolicy`

そのため、自動セットアップスクリプト（`setup-iam.sh`）が実行できません。

## ✅ 解決方法

管理者権限を持つユーザー、またはAWSコンソールを使用して手動でセットアップする必要があります。

---

## 📋 手順1: AWSコンソールでポリシーを作成

### 1-1. IAMコンソールを開く

1. [AWS IAM コンソール](https://console.aws.amazon.com/iam/)にアクセス
2. 左メニューから **「ポリシー」** を選択
3. **「ポリシーの作成」** をクリック

### 1-2. ポリシーJSONを入力

1. **「JSON」** タブを選択
2. 以下の内容をコピー＆ペースト：

```json
{
  "Version": "2012-10-17",
  "Statement": [
    {
      "Effect": "Allow",
      "Action": ["sns:*"],
      "Resource": "*"
    },
    {
      "Effect": "Allow",
      "Action": ["lambda:*"],
      "Resource": "*"
    },
    {
      "Effect": "Allow",
      "Action": [
        "iam:CreateRole",
        "iam:DeleteRole",
        "iam:GetRole",
        "iam:TagRole",
        "iam:UntagRole",
        "iam:PutRolePolicy",
        "iam:DeleteRolePolicy",
        "iam:GetRolePolicy"
      ],
      "Resource": "*"
    },
    {
      "Effect": "Allow",
      "Action": "iam:PassRole",
      "Resource": "*",
      "Condition": {
        "StringEquals": {
          "iam:PassedToService": "lambda.amazonaws.com"
        }
      }
    },
    {
      "Effect": "Allow",
      "Action": ["logs:*"],
      "Resource": "*"
    },
    {
      "Effect": "Allow",
      "Action": ["events:*"],
      "Resource": "*"
    }
  ]
}
```

3. **「次へ」** をクリック
4. ポリシー名: **`TerraformDailyCostMonitorPolicy`**
5. 説明（オプション）: `Policy for Terraform to manage Daily Cost Monitor resources`
6. **「ポリシーの作成」** をクリック

---

## 📋 手順2: ユーザーにポリシーをアタッチ

### オプションA: 既存ユーザー（canale.dev）に追加

1. IAMコンソールで **「ユーザー」** を選択
2. **`canale.dev`** をクリック
3. **「アクセス許可」** タブ
4. **「アクセス許可を追加」** → **「ポリシーを直接アタッチする」**
5. 検索欄に `TerraformDailyCostMonitorPolicy` と入力
6. チェックボックスをON
7. **「次へ」** → **「アクセス許可を追加」**

### オプションB: 新規ユーザーを作成（推奨）

1. IAMコンソールで **「ユーザー」** を選択
2. **「ユーザーを作成」** をクリック
3. ユーザー名: **`terraform-daily-cost`**
4. **「次へ」**
5. **「ポリシーを直接アタッチする」** を選択
6. `TerraformDailyCostMonitorPolicy` を検索して選択
7. **「次へ」** → **「ユーザーの作成」**

---

## 📋 手順3: アクセスキーを作成

### 3-1. アクセスキーの作成

1. 作成したユーザー（または `canale.dev`）のページを開く
2. **「セキュリティ認証情報」** タブ
3. **「アクセスキーを作成」** をクリック
4. ユースケース: **「Command Line Interface (CLI)」** を選択
5. 確認チェックボックスをON
6. **「次へ」**
7. 説明タグ（オプション）: `Terraform for daily-cost-monitor`
8. **「アクセスキーを作成」**

### 3-2. 認証情報を保存

⚠️ **重要**: アクセスキーとシークレットキーをコピーして安全に保管してください。

---

## 📋 手順4: AWS CLIの設定

### ターミナルで実行

```bash
# 新しいプロファイルを作成
aws configure --profile daily-cost-terraform

# 以下を入力
AWS Access Key ID: [上記で取得したアクセスキーID]
AWS Secret Access Key: [上記で取得したシークレットアクセスキー]
Default region name: ap-northeast-1
Default output format: json
```

または、直接ファイルを編集：

```bash
# ~/.aws/credentials に追加
cat >> ~/.aws/credentials <<EOF

[daily-cost-terraform]
aws_access_key_id = YOUR_ACCESS_KEY_ID
aws_secret_access_key = YOUR_SECRET_ACCESS_KEY
EOF

# ~/.aws/config に追加
cat >> ~/.aws/config <<EOF

[profile daily-cost-terraform]
region = ap-northeast-1
output = json
EOF
```

---

## 📋 手順5: Terraformを実行

```bash
cd /Users/canale/Documents/daily-cost

# 設定ファイルを作成
cp terraform.tfvars.example terraform.tfvars

# terraform.tfvars を編集
vim terraform.tfvars
# notification_email を設定

# プロファイルを設定
export AWS_PROFILE=daily-cost-terraform

# Terraform実行
terraform init
terraform plan
terraform apply
```

---

## 🔍 確認方法

### ポリシーが正しく作成されたか確認

```bash
aws iam get-policy --policy-arn arn:aws:iam::010526250511:policy/TerraformDailyCostMonitorPolicy
```

### ユーザーにポリシーがアタッチされているか確認

```bash
# 管理ポリシーの確認
aws iam list-attached-user-policies --user-name canale.dev

# または新規ユーザーの場合
aws iam list-attached-user-policies --user-name terraform-daily-cost
```

### 動作確認

```bash
# 認証情報の確認
aws sts get-caller-identity --profile daily-cost-terraform

# 出力例:
# {
#     "UserId": "AIDAXXXXXXXXXX",
#     "Account": "010526250511",
#     "Arn": "arn:aws:iam::010526250511:user/terraform-daily-cost"
# }
```

---

## 🎯 管理者への依頼メール文例

もし自分で実行できない場合、管理者に以下のような依頼メールを送ってください：

```
件名: Terraform実行用IAMポリシーの作成依頼

お世話になっております。

AWS Daily Cost Monitorシステムの構築にあたり、
Terraform実行用のIAMポリシーとユーザーの作成をお願いいたします。

■ 必要な作業
1. IAMポリシーの作成
   - ポリシー名: TerraformDailyCostMonitorPolicy
   - ポリシーJSON: 添付ファイル「iam-policy-compact.json」を参照

2. 以下のいずれかを実施
   a) 既存ユーザー（canale.dev）に上記ポリシーをアタッチ
   b) 新規ユーザー（terraform-daily-cost）を作成し、ポリシーをアタッチ

3. アクセスキーの発行（CLI実行用）

■ ファイル
- ポリシーJSON: iam-policy-compact.json
- 詳細手順: MANUAL_SETUP.md

よろしくお願いいたします。
```

---

## 📚 関連ファイル

- **iam-policy-compact.json** - コンパクト版ポリシー（推奨）
- **iam-policy-for-terraform.json** - 詳細版ポリシー
- **TERRAFORM_SETUP.md** - 詳細なセットアップガイド
- **QUICKSTART.md** - クイックスタートガイド

---

## ❓ よくある質問

### Q: なぜ自動スクリプトが使えないのか？

A: IAMポリシーやユーザーを作成するには、高い権限が必要です。セキュリティ上、これらの権限は管理者のみに付与されています。

### Q: canale.dev ユーザーに直接権限を追加できないか？

A: 可能ですが、以下の権限が必要です：
- `iam:CreatePolicy`
- `iam:AttachUserPolicy`
- `iam:PutUserPolicy`

これらは管理者権限レベルの操作です。

### Q: 別のアカウントでも使えるか？

A: はい。アカウントID（010526250511）は自動的に認識されるため、同じポリシーを他のAWSアカウントでも使用できます。

---

## 🆘 トラブルシューティング

### エラー: "Policy document is too large"

→ `iam-policy-compact.json` を使用してください（578文字）

### エラー: "AccessDenied"

→ 管理者権限を持つユーザーで実行するか、AWSコンソールで手動作成してください

### ポリシーが見つからない

→ 正しいAWSアカウントにログインしているか確認してください

---

**作成日**: 2024年12月
**更新日**: 2024年12月

